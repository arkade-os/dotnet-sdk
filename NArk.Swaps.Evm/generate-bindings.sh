...
# Router.sol (for the deferred USDT0-DEX-hop follow-up — executeAndLockERC20WithPermit2 /
# claimERC20ExecuteOft), IERC20.sol (generic ERC20 calls, e.g. approve), and TestERC20.sol
# (a deployable bytecode + typed constructor, for local Anvil test fixtures that need their
# own throwaway token — not used by the production provider, which only ever calls
# already-deployed contracts). EtherSwap.sol is vendored for reference but not generated
# (native-ETH sibling contract, not used by our ERC20/tBTC flow).
set -euo pipefail
