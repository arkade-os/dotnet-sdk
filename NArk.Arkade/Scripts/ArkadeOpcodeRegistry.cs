...
public static class ArkadeOpcodeRegistry
{
    ...
    static ArkadeOpcodeRegistry()
    {
        ...
        // <c>0xb3</c> + <c>0xc3–0xf8</c> range and don't collide with anything
        ...
        foreach (var opcode in Enum.GetValues<NBitcoin.OpcodeType>())
        {
            ...
            // Skip bare aliases that are themselves valid hex, i.e. the even-length numeric
            // ones ("10".."16" for OP_10..OP_16). FromAsm consults this table before falling
            // back to hex, so registering them would silently turn a one-byte data push of
            // 0x10-0x16 into a small-integer opcode. The OP_-prefixed form still resolves via
            // TryParseOpN. Odd-length numeric aliases ("1".."9") can never be a hex token, so
            // they stay registered and the bare form documented in the README keeps working.
            if (bareName.Length % 2 != 0 || !bareName.All(char.IsAsciiDigit))
