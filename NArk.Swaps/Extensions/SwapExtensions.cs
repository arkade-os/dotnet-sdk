...
public static class SwapExtensions
{
    ...
    /// <summary>
    /// Returns the cached on-chain BTC refund destination from swap metadata, or derives a
    /// fresh boarding contract and caches it. This is the on-chain counterpart of
    /// <see cref="GetOrDeriveRefundDestinationAsync"/>: BTC→ARK chain-swap refunds land on
    /// L1, so the destination must be a genuine wallet-owned Bitcoin address rather than an
    /// Ark address. Deriving a boarding contract (instead of e.g. reusing the swap's own BTC
    /// lockup address) means the refunded coins are immediately usable — collaboratively
    /// with the Ark server, or unilaterally after the exit delay.
    /// </summary>
    /// <returns>The on-chain address to refund to, and the (possibly updated) swap record.</returns>
    public static async Task<(BitcoinAddress Destination, ArkSwap Swap)> GetOrDeriveBtcRefundDestinationAsync(
        this ArkSwap swap,
        IContractService contractService,
        ISwapStorage swapStorage,
        Network network,
        CancellationToken ct)
    {
        if (swap.Get(SwapMetadata.BtcRefundAddress) is { } cached)
            return (BitcoinAddress.Create(cached, network), swap);
        ...
        var contract = await contractService.DeriveContract(
            ...
            metadata: new Dictionary<string, string> { ["Source"] = $"swap-refund:{swap.SwapId}" },
            cancellationToken: ct)
            as ArkBoardingContract
            ?? throw new InvalidOperationException($"Swap {swap.SwapId}: failed to derive BTC refund destination");

        var addr = contract.GetOnchainAddress(network);
        var updated = swap with
        {
            Metadata = new Dictionary<string, string>(swap.Metadata ?? [])
            { [SwapMetadata.BtcRefundAddress] = addr.ToString() },
            UpdatedAt = DateTimeOffset.UtcNow
        };
        await swapStorage.SaveSwap(swap.WalletId, updated, ct);
        return (addr, updated);
