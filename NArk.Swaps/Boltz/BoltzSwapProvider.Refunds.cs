...
public partial class BoltzSwapProvider
{
    internal async Task RequestSubmarineCoopRefund(ArkSwap swap, SwapStatusResponse swapStatus, CancellationToken cancellationToken = default)
    {
        ...
        // The snapshot and includeSpent query cover the contract's complete history.
        ...
        var contractHistory = await _vtxoStorage.GetVtxos(
            scripts: [swap.ContractScript],
            includeSpent: true,
            cancellationToken: cancellationToken);
        var canonicalHistory = contractHistory
            .Where(v => (long)v.Amount == swap.ExpectedAmount)
            .ToArray();
        var vtxo = canonicalHistory.FirstOrDefault(v => !v.IsSpent());
        ...
        if (vtxo is null)
        {
            if (canonicalHistory.Length > 0)
            {
                _logger?.LogInformation(
                    "Swap {SwapId}: canonical lockup was funded but is already spent; refusing to classify it as unfunded",
                    swap.SwapId);
                return;
            }
            ...
            _logger?.LogWarning(
                "Swap {SwapId}: no canonical {ExpectedAmount}-sat lockup found in contract history",
                swap.SwapId, swap.ExpectedAmount);
            return;
        }
        ...
        var extraUnspentCount = contractHistory.Count(v => !v.IsSpent()) - 1;
        if (extraUnspentCount > 0)
        {
            _logger?.LogInformation(
                "Swap {SwapId}: refunding the canonical lockup and leaving {Extras} extra VTXO(s) for the sweeper",
                swap.SwapId, extraUnspentCount);
        }
